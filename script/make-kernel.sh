...
BUILD_PATH="../build_linux_6.1.115"
...
KERNEL_DEFCONFIG="$KERNEL_DEFCONFIG rk3576_poc.config"
...
BOOT_CMDLINE="console=ttyFIQ0 firmware_class.path=/vendor/etc/firmware init=/init rootwait ro loop.max_part=7 androidboot.console=ttyFIQ0 androidboot.wificountrycode=KR androidboot.hardware=rk30board androidboot.boot_devices=2a330000.mmc androidboot.selinux=permissive"
SECURITY_LEVEL="2022-03-05"

mkbootfs -d ../out/target/product/$PRODUCT_DEVICE/system ../out/target/product/$PRODUCT_DEVICE/ramdisk | gzip -9 -c > ../out/target/product/$PRODUCT_DEVICE/ramdisk.img 
...
## copy to build directory
mkbootimg --kernel $BUILD_PATH/arch/arm64/boot/Image --ramdisk ../out/target/product/$PRODUCT_DEVICE/ramdisk.img --dtb $BUILD_PATH/arch/arm64/boot/dts/rockchip/$KERNEL_DTS.dtb --cmdline "$BOOT_CMDLINE" --os_version 15 --os_patch_level $SECURITY_LEVEL --second $BUILD_PATH/resource.img --header_version 2 --output $BUILD_PATH/boot.img
#mkbootimg --kernel $BUILD_PATH/arch/arm64/boot/Image --ramdisk ../out/target/product/$PRODUCT_DEVICE/ramdisk.img --dtb $BUILD_PATH/arch/arm64/boot/dts/rockchip/$KERNEL_DTS.dtb --cmdline "$BOOT_CMDLINE" --second $BUILD_PATH/resource.img --header_version 2 --output $BUILD_PATH/boot.img
